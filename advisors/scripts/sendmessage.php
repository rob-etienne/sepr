    	...
    	if (!empty($errSubject))
      	{
			...
			$log->messageFailure("Invalid subject field.", $_SESSION['EmployeeNr']);
    	...
    	elseif (!empty($errMessage))
      	{
			...
			$log->messageFailure("Invalid message field.", $_SESSION['EmployeeNr']);
			header('Location: ../account.php');
			exit();
      	}
		elseif (!empty($errFile))
      	{
			...
			$log->messageFailure("Invalid file upload.", $_SESSION['EmployeeNr']);
			header('Location: ../account.php');
			exit();
      	}
		elseif (!empty($errCSRF))
      	{
			...
			$log->messageFailure("Invalid csrf token.", $_SESSION['EmployeeNr']);
			header('Location: ../account.php');
			exit();
      	}
   	}
...
{	
	...
	if($fileToUpload)
	{
		...
		if (!move_uploaded_file($_FILES["upload"]["tmp_name"], $target_file)) 
		{
			...
			$log->messageFailure($errFile, $_SESSION['EmployeeNr']);
		}
		else
		{
	...
	if($result)
	{
		...
		$log->messageSuccess($_SESSION['EmployeeNr']);
	}
	else
	{
		// Sending message failed
		$_SESSION['error'] = "Error occured while sending message.";
		$log->messageFailure("Database error",$_SESSION['EmployeeNr']);
	}
